/*!
 * \file
 * \brief Файл исходников с реализацией всех необходимых функций.
 */
#include <datatable.h>
#include <QString>
#include <QDialog>
...
bool operator==(const DataTable &L, const DataTable &R)
{
    QList<QString> VL, VR;
    VL = L.ColName;
    VR = R.ColName;
    ...
    QList<QList<QString>> VL1, VR1;
    VL1 = L.Data;
    VR1 = R.Data;
    if (VL1.length() != VR1.length()) {
        return false;
    }
    for (int i = 0; i < VL1.length(); i++) {
        for (int j = 0; j < VL1[i].length(); i++) {
            if (VL1[i][j] != VR1[i][j]) {
                return false;
            }
        }
